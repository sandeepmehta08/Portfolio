<template>
  <div v-bind="$attrs">
    <div
      class="heading-2 max-sm:heading-3 flex min-h-[5.5rem] grid-cols-12 items-center justify-start gap-5 py-8 text-left font-semibold sm:min-h-[6.5rem] sm:py-6 md:grid md:justify-between max-sm:min-h-[4.5rem] max-sm:py-5"
    >
      <span class="col-span-2 self-center text-nowrap">
        ( {{ `0${number}` }} )
      </span>
      <h3 class="col-span-6 col-start-5 text-balance pr-2">{{ title }}</h3>
      <div
        v-html="shape"
        ...
        <div>
          <template v-for="(heading, index) in headings" :key="index">
            <p
              ...
              :class="{
                'border-y border-flax-smoke-500/50 py-3!': index === 1,
              }"
            >
              <span
